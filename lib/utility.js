...
module.exports.inspectors = {
  Type: {
    ...
    ArrayType: function(depth){
      var label = color.byellow(this.name, '‹›') + color.bblue('('+this.bytes+'b)');
      var memberType = util.inspect(this.memberType, false, depth-1);
  ...
  Data: {
    NumericType: function(){
      return color.magenta(this.Subtype, '<>')+' '+color.bmagenta(this.reify());
    },
    ArrayType: function(depth){
