...
module.exports.bitfieldInspect = function inspect(depth){
  var label = '{ [Bitfield: '+this.length+'bits] '
  if (!this.flags.length) {
    var bits = this.map(function(val){ return +val });
    return label + bits.join('')+' }'
  } else {
    var length = strlen(label);
    var fields = this.flags.map(function(flag, index){
      flag = flag+': ' + this[flag];
      length += strlen(flag);
      return flag;
    }, this);
    var sep = length > 60 ? '\n  ' : ' ';
    return label+sep+fields.join(','+sep)+' }';
  }
}