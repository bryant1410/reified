...
var initArray = require('./genesis').initType.bind(ArrayType);
...
function ArrayType(name, MemberType, length) {
  if (typeof name !== 'string') {
    length = MemberType || 0;
    MemberType = genesis.lookup(name) || genesis.Type;
    name = MemberType.name + 'x'+length;
  } else {
    MemberType = genesis.lookup(MemberType);
  }
  ...
  var ArrayTInterface = Object.defineProperties(initArray(name, ArrayT), {
  ...
  Object.defineProperties(ArrayTInterface.prototype, {
    length: D.EC_(length),
  });
