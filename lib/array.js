...
function ArrayType(name, type, length) {
  if (typeof name !== 'string') {
    length = type;
    type = name;
    name = type.name.replace(/Array$/, '') + 'x' + length + 'Array';
  }
...
ArrayType.prototype.prototype = {
  ...
  inspect: function inspect(){
    ...
    return '[ [Array: '+this.constructor.name+']'+sep+this.map(function(item){ return item.inspect() }).join(','+sep)+' ]';
  }
};
