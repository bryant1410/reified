...
function ArrayType(elementType, length) {
  ...
  var ctor = function ArrayConstructor(values){
    return ArrayAllocate(ctor, ctor.prototype, values);
  }
...
function ArrayAllocate(ctor, target, values){
  ...
  Array.apply(null, Array(ctor._Length)).forEach(function(v,i){
    Object.defineProperty(target, i, {
      enumerable: true,
      configurable: true,
      get: function(){
        return ctor.elementType._Reify(target._Value[i]);
      },
      set: function(v){
        var R = ctor.elementType._Convert(v);
        target._Value[i] && target._Value[i].write(R._Value ? R._Value.deref() : 0);
      }
    });
    target[i] = values ? values[i] : 0;
  });
