...
function ArrayType(elementType, length) {
  ...
  var ctor = function ArrayConstructor(values, buffer){
    return ArrayAllocate(ctor, Object.create(ctor.prototype), values, buffer);
  }
  ...
  function Convert(val){
    if (isObject(val) && val.toString() === '[object Block]') {
  ...
  function IsSame(compare){
    return compare._DataType === 'array' &&
           ctor.elementType._IsSame(compare.elementType) &&
           ctor._Length === compare._Length;
  }
  ...
  function Reify(R){
    return Object.create(ctor.prototype, {
      _Value:    D._C_(R),
      _DataType: D._C_(ctor),
      length:    D.EC_(ctor._Length),
    });
  }
  ...
  function fill(val){
    if (i.isObject(this) || thos.toString() !== '[object Data]' || !this._DataType._IsSame(ctor)) {
      throw new TypeError('Method is not generic');
    }
    sLoop(this._Length, function(i){
      var allocated = self.elementType._Convert(val);
      self[i] = allocated._Value;
    });
  }
   ...
   */
  function ArrayAllocate(target, values, buffer){
    Object.defineProperties(target, {
      _Value: D._C_(new ArrayBlock(ctor, values, buffer)),
    });
    return target;
  }
...
function repeat(val){
  if (!isObject(this) || this.toString() !== '[object DataType]' || this._DataType !== 'array') {
    throw new TypeError('Method is not generic');
  }
  var V = new this(val);
  V.fill(val);
