...
module.exports = NumericInterface;
...
function NumericType(name, bytes){
  ...
  function NumericT(buffer, offset, value){
    if (!NumericInterface.isInstance(this)) {
...
function NumericInterface(type, buffer, offset, val){
  if (!NumericInterface.isNumeric(this) && typeof buffer === 'number') {
    // Call with number -> `cast(type, number)`
    return castToType(type, buffer);

  } else if (type in NumericInterface) {
    // Call/construct specified type -> `new BlockType`
    return new NumericInterface[type](buffer, offset, val);

  } else if (NumericInterface.isNumeric(type)) {
    // Call with existing block -> `block.reify()`
    return type.reify();
  }
}

NumericInterface.isInstance = function isInstance(o){
  return NumericData.isPrototypeOf(o);
}



Object.keys(types).forEach(function(name){
  NumericInterface[name] = new NumericType(name, types[name][0]);
});
