...
var isMemory = require('./mapped-memory').isMemory;
...
// to produce instance versions from. It has no mapping to any real memory.
...
states.free = {
  ...
  // Apply to convert this into an allocated type
...
states.allocated = {
  getOwnPropertyDescriptor: function(t, name){
    if (name in this.fields) {
      return normal(this.memory.get(props[name]));
    }
  },
  defineProperty: function(t, name, desc){
    if (name in this.fields) {
      this.memory.set(this.fields[name], desc.value);
      return true;
    }
  },
  deleteProperty: function(t, name){
    if (name in this.fields) {
      this.memory.set(this.fields[name], null);
    }
    return true;
  },
  get: function(t, name, recvr){
    if (name in this.fields) {
      return this.memory.get(this.fields[name]);
    }
  },
  set: function(t, name, val, recvr){
    if (name in this.fields) {
      this.memory.set(this.fields[name], val);
    }
  },
  //TODO the next four
  //apply: function(t, recvr, args){ },
