...
var initBitfield = require('./genesis').initType.bind(BitfieldType);
...
function BitfieldType(name, flags, bytes){
  ...
  BitfieldT.bytes = bytes;

  // ###########################
  // ### BitfieldT Interface ###
  // ###########################

  var BitfieldInterface = initBitfield(name, BitfieldT);
  ...
  Object.defineProperties(BitfieldInterface.prototype, {
    flags:  D._CW(flags),
    length: D._C_(flags.length > 0 ? flags.length : bytes * 8),
  });

  defineFlags(BitfieldInterface.prototype, flags);
