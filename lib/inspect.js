...
var inspectors = {
  Type: {
    ...
    PointerType: function(object, showHidden, depth, useColor){
      if (Object.prototype.hasOwnProperty.call(object, 'Type')) return '[PointerType Prototype]';
      return color.bred('*'+object.name, '‹›')+ color.bblue('('+object.bytes+'b)') + '\n'+
            util.inspect(object.pointeeType, showHidden, depth-1, useColor);
    },
  ...
  Data: {
    ...
    PointerType: function(object, showHidden, depth, useColor){
      if (!object.reify) return color.bred('[PointerData Prototype]');
      var label = color.red('*'+object.constructor.pointeeType.name, '<>');
      return label+'\n'+ util.inspect(object.pointee, showHidden, depth-1, useColor).slice(label.length);
    },
    CharArrayType: function(object, showHidden, depth, useColor){
      if (object.bytes === 1) {
