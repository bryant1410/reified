...
function StructType(name, fields){
  ...
  function StructConstructor(buffer, offset, val){
    ...
    Object.defineProperty(this, 'buffer', D._CW(new DataView(buffer, offset || 0, bytes)));
...
function inspect(){
  ...
  var fields = this.constructor.names.map(function(field){
                 field +=': '+this[field].inspect();
                 length += field.length;
                 return field;
               }, this);

  var sep = length > 80 ? '\n  ' : ' ';
  return  '{ «Struct: '+this.constructor.name+'»'+sep+fields.join(','+sep)+' }';
}
